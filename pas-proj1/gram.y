...
%type <y_type> array_type typename subrange_type new_ordinal_type type_denoter pointer_domain_type  new_pointer_type ordinal_index_type functiontype new_procedural_type
%type <y_int>  constant number unsigned_number
%type <y_string> new_identifier new_identifier_1
%type <y_index_list> array_index_list
%type <y_member_list> id_list
%type <y_param_list> procedural_type_formal_parameter_list procedural_type_formal_parameter optional_procedural_type_formal_parameter_list 
...
/* GPC internal tokens */
%token LEX_STRCONST LEX_REALCONST
...
new_procedural_type:
    LEX_PROCEDURE optional_procedural_type_formal_parameter_list {$$ = tree_build_func(NULL, $2); }
  | LEX_FUNCTION optional_procedural_type_formal_parameter_list functiontype  { $$ = tree_build_func($3, $2);}
  ;

optional_procedural_type_formal_parameter_list:
    /* empty */										{ $$ = NULL ;/*PARAM_LIST x = NULL ; $$ = x */}
  | '(' {tree_var_head_init();} procedural_type_formal_parameter_list ')'   { $$ = $3 ;}
  ;
